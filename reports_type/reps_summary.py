...
'''
    ...
    if "Role" in columns and "Visits" in columns and "Total revenue":
        with st.container(border=True):
            col11, col12 = st.columns([10, 0.50])
            with col11:
                st.markdown("""
        ...
        st.warning("There is no Total revenue or Visits or Role columns, so visualizing can not be ready")
    
    if "Role" in columns and "Orders total" in columns and "Total revenue":
        with st.container(border=True):
            col11, col12 = st.columns([10, 0.50])
            with col11:
                st.markdown("""
